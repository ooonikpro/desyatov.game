...
const playerModel = createModel<PlayerModelStateType, PlayerModelMethodsType>((set, get) => {
  const setPlaylist = (playlist: PlayerModelStateType["playlist"]) => set((state) => (state.playlist = playlist));
  ...
  const setIsShowControls = (val: boolean) => set((state) => (state.isShowControls = val));
  const setIsShowPlaylist = (val: boolean) => set((state) => (state.isShowPlaylist = val));
